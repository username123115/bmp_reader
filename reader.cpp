#include <iostream>
#include <fstream>
#include <iomanip>
#include <cmath>

#pragma pack (push, 1)
struct Header {
    uint16_t signature;
    uint32_t size;
    uint16_t r1;
    uint16_t r2;
    uint32_t offset;
};

#pragma pack(push, 1)
struct BMP_info_header {
    uint32_t h_size;
    uint32_t w;
    uint32_t h;
    uint16_t planes;
    uint16_t bpp;
    uint32_t compression;
    uint32_t size; //image size
    uint32_t xppm;
    uint32_t yppm;
    uint32_t color_table_colors;
    uint32_t import_colors;
};

int max(int a, int b) {
    if (a > b) {
        return a;
    }
    else {
        return b;
    }
}

using namespace std;
int main(int argc, char* argv[]) {
    ifstream image("bitmap_test.bmp", ios_base::in | ios_base::binary);
    ofstream output("bitmap_output.bmp", ios_base::binary);
    if (!image.is_open()) {
        cout << "Image did not open" << endl;
        return 1;
    }   
    else {
        Header image_header;
        BMP_info_header info_header;
        image.read((char *)&image_header, sizeof(Header));
        if (image_header.signature == 0x4d42) {
            image.read((char *)&info_header, sizeof(info_header));
            
            int chart_values = (image_header.offset - 12 - info_header.h_size) / 4; //values start from 54 to offset and each entry contains 4 bytes
            uint32_t color_table[chart_values][4]; // split entrys into r, g, b, and fill
            
            int bytes_per_pixel = info_header.bpp / 8;
            int padding = 4 - (info_header.w % 4);
            if (padding == 4) {
                padding == 0;
            }
            char pixels[info_header.h][(info_header.w + padding) * bytes_per_pixel];
            char out_pixels[info_header.h][(info_header.w + padding) * bytes_per_pixel];

            image.read((char *)&color_table, sizeof(color_table));
            image.read((char *)&pixels, sizeof(pixels));
            //inverting colors
            for (int i = 0; i < info_header.h; i++) {
                for (int j = 0; j < (info_header.w + padding) * bytes_per_pixel; j++) {
                    out_pixels[i][j] = max(256, pow(2, info_header.bpp)) - 1 - pixels[i][j];
                }
            }


            output.write((char*)&image_header, sizeof(Header));
            output.write((char*)&info_header, sizeof(BMP_info_header));
            output.write((char*)&color_table, sizeof(color_table));
            output.write((char*)out_pixels, sizeof(pixels));
            image.close();
            return 0;
        }
        else {
            cout << "Wrong file format" << endl;
            image.close();
            return 1;
        }
    }

}